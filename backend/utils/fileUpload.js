...
const categoryStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const uniqueName = 'category-' + Date.now() + path.extname(file.originalname);
    cb(null, uniqueName);
  }
});
...
const subCategoryStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const uniqueName = 'subcategory-' + Date.now() + path.extname(file.originalname);
    cb(null, uniqueName);
  }
});
...
const fileFilter = (req, file, cb) => {
  ...
  allowedTypes.includes(ext) ? cb(null, true) : cb(new Error('Apenas imagens são permitidas (JPEG, PNG, SVG)'), false);
};
